(ns pubmed.main
  "Kotodama WASM entrypoint for the Pubmed clean-room actor (L4) — Clojure port.
  ...
  facts (`pubmed.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "pubmed")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"pubmed.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "pubmed-compat" :tier tier :entities entities} 200])
...
(defn start! [] :pubmed-compat/ready)
